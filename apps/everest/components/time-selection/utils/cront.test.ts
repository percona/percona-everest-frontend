...
describe('cron', () => {
    describe('getCronStringFromValues', () => {
        it('should generate wildcards if only period passed', () => {
            ...
            expect(getCronStringFromValues('hour', [], [], [], [1], [])).toBe('* * 7 * *');
        });
        ...
        describe('year period', () => {
            it('should generate correct expressions', () => {
                ...
                expect(getCronStringFromValues('year', [1, 2, 3], [10, 23], [0, 2], [10], [15, 30])).toBe(
                    '15,30 10 10,23 1-3 0,2'
                );
                expect(getCronStringFromValues('year', [1, 2, 3], [10, 23], [0, 1, 2, 3, 4, 5, 6], [], [])).toBe(
                    '* * 10,23 1-3 *'
                );
            });
        });
        ...
        describe('day period', () => {
            it('should generate correct expressions', () => {
                expect(getCronStringFromValues('day', [], [], [], [1], [15])).toBe('15 1 * * *');
                expect(getCronStringFromValues('day', [], [], [], [], [15])).toBe('15 * * * *');
            });
        });
    });
