...
import { ProxyExposeType } from '../../src/types/dbCluster.types';
import { dbTypeToDbEngine } from '../../src/utils/db';
...
export const createDbClusterFn = async (request, customOptions, kubernetesId) => {
            ...
            proxy: {
                ...
                expose: {
                    ...
                    ...(!!customOptions?.externalAccess &&
                        ...
                        ipSourceRanges: customOptions?.sourceRanges.flatMap((source) => source.sourceRange ? [source.sourceRange] : []),
                    }),
                },
            },
    ...
    const response = await request.post(`/v1/kubernetes/${k8sClusterId}/database-clusters`, {
        data: payload,
    });
    expect(response.ok()).toBeTruthy();
};
export const deleteDbClusterFn = async (request, kubernetesId, clusterName) => {
    const deleteResponse = await request.delete(`/v1/kubernetes/${kubernetesId}/database-clusters/${clusterName}`);
    expect(deleteResponse.ok()).toBeTruthy();
};
//# sourceMappingURL=db-cluster.js.map