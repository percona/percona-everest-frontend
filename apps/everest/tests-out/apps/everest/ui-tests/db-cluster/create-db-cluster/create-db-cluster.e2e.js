...
test.describe('DB Cluster creation', () => {
    ...
    test.beforeAll(async ({ request }) => {
        ...
        const { storageClassNames = [] } = await getClusterDetailedInfo(request, kubernetesId);
    ...
    test('Cluster creation', async ({ page, request }) => {
        ...
        await basicInformationStepCheck(page, engineVersions, storageClasses, clusterName);
    ...
    test.skip('Cancel wizard', async ({ page }) => {
        ...
        //
        // await page.getByTestId('db-wizard-continue-button').click();
        await expect(page.getByRole('heading', { name: 'Advanced Configurations' })).toBeVisible();
...
//# sourceMappingURL=create-db-cluster.e2e.js.map