...
test.describe('Backup storage', () => {
    ...
    test('Backup storage creation', async ({ page }) => {
        ...
        await page.getByTestId('text-input-bucket-name').fill(EVEREST_LOCATION_BUCKET_NAME);
        ...
        await page.getByTestId('text-input-access-key').fill(EVEREST_LOCATION_ACCESS_KEY);
        await page.getByTestId('text-input-secret-key').fill(EVEREST_LOCATION_SECRET_KEY);
        ...
        await page
            ...
            .filter({ hasText: 'test-storage-name' }).getByTestId('MoreHorizIcon')
            .click();
